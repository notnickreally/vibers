...
/** What Next reads off an init to decide whether this leg may be served a cached answer. */
type Reads = { cache: "no-store" } | { next: { revalidate: number } };

/**
 * How one upstream read is allowed to be cached, by who asked for the sweep.
 *
 ...
 */
async function feedIds(channelId: string, fresh: boolean): Promise<string[] | null> {
  const url = feedUrl(channelId);
  if (!url) return [];
  try {
    const res = await fetch(url, {
      headers: { accept: "application/atom+xml,text/xml" },
      ...reads(fresh, FEED_TTL),
    });
...
async function youtubeLive(
  ...
  for (const channel of channels) {
    const ids = await feedIds(channel.id, fresh);
    if (ids === null) {
