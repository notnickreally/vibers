 ...
 * too, and it must run the *same* one. The module-level memo and quota latch
 * below only do their job if there is a single copy of them — two modules each
 ...
 *   knows.
 *
 * It takes **no input**. That is deliberate: a caller-supplied keyword would
 * let any visitor spend a scarce site-wide resource that costs them nothing
 * and cannot be topped up. Keywords come from the deployment, or from the
 * seeds.
 *
